...
bool GLShader::Load(const char* filePath)
{
	ShaderFileParser sfp;
	sfp.LoadShaderFile(filePath);
	sfp.Parse();
	const std::vector<Shader>& shaders = sfp.mShaders;
	for (int i = 0; i < shaders.size(); ++i)
	{
		switch (shaders[i].mType)
		{
		case Vertex:
			if (!CompileShaderFromString(shaders[i].mSource, GLSLShader::VERTEX))
			{
				printf("Vertex Shader Failed to compile!\n %s", GetLog().c_str());
				exit(1);
			}
			break;
		case Geometry:
			if (!CompileShaderFromString(shaders[i].mSource, GLSLShader::GEOMETRY))
			{
				printf("Geometry Shader Failed to compile!\n %s", GetLog().c_str());
				exit(1);
			}
			break;
		case Fragment:
			if(!CompileShaderFromString(shaders[i].mSource, GLSLShader::FRAGMENT))
			{
				printf("Fragment Shader Failed to compile!\n %s", GetLog().c_str());
				exit(1);
			}
